...
// Part 3 Notes:
// 1. Don't send SIGUSR1/SIGSTOP/SIGCONT as per part 2.

// 2. Create the following global variables : PCB * to hold array of program control blocks, int ProgramCount for # programs, int CurrentProgram for the currently active program, int StartRun for child processes to trigger on *this is actually needed for part 2*.

// 3. Create an enum State for the state of the programs, using descriptive words like: NotStarted, Running, Paused, Exited. Initialize all PCB's to NotStarted.

// 4. Add two members to the PCB structure: enum State, and int HasExited.

// 5. Add 2 signal handlers:  void SIGCHLD(int signal), void SIGALRM(int signal).

// 6. Create a helper function to loop over all programs, and return true/1 if all PCB.HasExit is true. *all children exited*

// 7. Create a function to wait on signal using sigwait.  https://stackoverflow.com/questions/6326290/about-the-ambiguous-description-of-sigwait
...
//  Other persons psuedocode:

//  sigChild(){
//         loop over all procs
//         check if proc is exited
//                 if it is, update exited state
//         //make the below a function
//         loop over all procs again,
//         if all exited, call kill(sigusr2) to self;
// }
...
// From Roscoe:
// 1. Check if all exited in SIGALARM

// 2. waitpid(pid,&status,WNOHANG) >0
